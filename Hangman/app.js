    ...
    $("button").on("click", function(){
        ...
        for (var i = 0; i < randomWord.length; i++) {
            if (userGuess === randomWord.charAt(i)) {
                $('#container').find(":nth-child(" + (i + 1) + ")").css("color", "black").addClass("winner");
                matchFound = true;
            }
        }
        ...
        $(".letter").each(function( index ) {
            if ( $(this).hasClass("winner") ) {
                ...
                if (goodGuesses.length === randomWordArray.length) {
                    ...
                    $(".category").text("Congratulations! You win");
        ...
        if (matchFound === false) {
            ...
            // $("#top").html("With " + wrongGuesses + " failures:");
        }
        ...
        if (wrongGuesses === 6) {
            $("#container").hide();
            $("button").prop("disabled", "true");
            $(".category").text("Better luck next time! The secret word was " + randomWord);
            $(".category").append("<br><button enabled class='play-again'>Play again?</button>");
        }
